...
contract MilkMaker {
    ...
    constructor(IUniswapV2Factory _factory, address _bar, address _milk, address _weth) public {
        factory = _factory;
        milk = _milk;
        bar = _bar;
        weth = _weth;
    ...
    function _toMILK(uint256 amountIn) internal {
        IUniswapV2Pair pair = IUniswapV2Pair(factory.getPair(weth, milk));
        (uint reserve0, uint reserve1,) = pair.getReserves();
        address token0 = pair.token0();
